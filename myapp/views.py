    ...
    if request.method == 'POST':
        ...
        if EmpCode:
            ...
            if "11:30" <= prac_time <= "13:30" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakout_timestamps >= timedelta(seconds=7):
                    employee_instance = Employee.objects.get(EmpCode=EmpCode)
...
#     now = timezone.now()
#     # Load the last execution time from cache
#     last_execution_time = cache.get('last_reset_execution_time')

#     # If it's the first time the function is called or a month has passed since the last execution
#     if last_execution_time is None or (now - last_execution_time) >= timedelta(days=30):
#         # Update the last execution time in cache to now
#         cache.set('last_reset_execution_time', now, timeout=None)
...
#         if now.day == first_day_of_month.day:
#             AttendanceCount.objects.update(GracePeriod=15)
#             print("SUCCESS RESET GRACE PERIOD ********************")
#     else:
#         print("Function already executed this month. Skipping...")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user:
                ...
                elif request.user.username == 'SERVIAMUS':
                    return redirect('serviamus_main_page')
                elif request.user.username == 'UTILITY_MAIN':   
                    return redirect('utility_mh_render_page')
                elif request.user.username == 'MIS_XYRYL':   
                    return redirect('home')
                elif request.user.username == 'HPC':   
                    return redirect('hpc_main_page')
                elif request.user.username == 'SGI':   
                    return redirect('main_sgi_page')
                elif request.user.username == 'NAZARETH':
                    return redirect('main_nazareth_page')
                elif request.user.username == 'FCH_SILAY' or request.user.username == 'EMB_TALISAY':
                    return redirect('testing_main_page')
                else:
