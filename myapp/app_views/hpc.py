...
@csrf_exempt
def fetch_messages_hpc(request):
    ...
    filtered_messages = [
        ...
        or 'no_bibo_hpc' in message.tags or 'breakin_already' in message.tags or 'timeout_already' in message.tags 
        or 'halfday_hpc' in message.tags or 'breakin_afternoon_hpc' in message.tags
    ]
    return JsonResponse({'messages': filtered_messages})


@csrf_exempt
def webcam_qr_code_scanner_hpc(request):
    if request.method == 'POST':
        image_data = request.FILES['webcam_image_hpc'].read()
        ...
        if decoded_objects:
            ...
            #FOR BREAKOUT
            if "12:00" <= prac_time <= "15:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_timein_timestamps >= timedelta(seconds=6):
            ...
            #FOR BREAKIN
            if "12:00" <= prac_time <= "15:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=True, date=current_time.date()).exists():
            ...
            #FOR TIMEOUT
            if "17:00" <= prac_time <= "23:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
            ...
            if "17:00" <= prac_time <= "23:59" and temporay.objects.filter(Q(breakin_names__isnull=True) | Q(breakout_names__isnull=True), EmpCode_id=EmpCode,timein_names__isnull=False, date=current_time.date()).exists():
                nobreak_out_in(EmpCode, current_time)
                messages.success(request, f'NO BREAKIN OR BREAKOUT!<br>{full_name}', extra_tags='no_bibo_hpc')
            ...
            if "12:00" <= prac_time <= "23:59":     
                employee_instance = Employee.objects.get(EmpCode=EmpCode)
                existing_entry = DailyRecord.objects.filter(EmpCode_id=EmpCode,date=current_time.date()).first()
                if existing_entry is None: 
                    messages.success(request, f'BREAK IN SUCCESSFULLY<br>{full_name}', extra_tags='breakin_afternoon_hpc')  
...
def timeout(employee_number,current_time):
    ...
    if current_time:
        ...
        upper_bound_timeout = datetime.combine(current_time.date(), time(18, 00, 0))
...
def hpc_main_page(request):
    return render(request, "myapp/HPC.html")
