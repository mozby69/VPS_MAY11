...
@csrf_exempt
def webcam_qr_code_scanner_serviamus_seven(request):
        ...
        if decoded_objects:
            ...
            if "04:00" <= prac_time <= "09:59": 
                ...
                if existing_entry is None: 
                    ...
                    messages.success(request, f'TIME IN SUCCESSFULLY!<br> {full_name}', extra_tags='timein_seven')
            ...
            #FOR BREAKOUT
            if "12:00" <= prac_time <= "14:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_timein_timestamps >= timedelta(seconds=7):
                    ...
                    messages.success(request, f'BREAK OUT SUCCESSFULLY<br>{full_name}', extra_tags='breakout_seven')
            ...
            #FOR BREAKIN
            if "12:00" <= prac_time <= "14:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakout_timestamps >= timedelta(seconds=8):
                    ...
                    messages.success(request, f'BREAK IN SUCCESSFULLY!<br> {full_name}', extra_tags='breakin_seven')  
            ...
            if "12:00" <= prac_time <= "14:00" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=8):
                    messages.success(request, f'BREAK IN ALREADY<br>{full_name}', extra_tags='breakin_already_seven')
                    return HttpResponseRedirect(request.path)
            ...
            if "15:00" <= prac_time <= "23:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=5):
                    ...
                    messages.success(request, f'TIME OUT SUCCESSFULLY<br>{full_name}', extra_tags='timeout_seven')    
            ...
            if "15:00" <= prac_time <= "23:59" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=6):
                    messages.success(request, f'TIMEOUT ALREADY!<br>{full_name}', extra_tags='timeout_already_seven')
                    return HttpResponseRedirect(request.path)
...
def timeout(employee_number,current_time):
    ...
    if current_time:
        ...
        upper_bound_timeout = datetime.combine(current_time.date(), time(16, 00, 0))
