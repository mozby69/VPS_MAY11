...
@csrf_exempt
def fetch_messages_utility_twelve(request):
  
    messages = get_messages(request)
    filtered_messages = [
        {'text': message.message, 'tags': message.tags} for message in messages if 'timein_twelve' in message.tags
        or 'breakout_twelve' in message.tags or 'breakin_twelve' in message.tags or 'timeout_twelve' in message.tags
        or 'no_bibo_utility_twelve' in message.tags or 'breakin_already_twelve' in message.tags or 'timeout_already_twelve' in message.tags 
...
@csrf_exempt
def webcam_qr_code_scanner_serviamus_utility_12(request):
    if request.method == 'POST':
        image_data = request.FILES['webcam_image_utility'].read()
        ...
        if decoded_objects:
            ...
            if "04:00" <= prac_time <= "09:59": 
                ...
                if existing_entry is None: 
                    ...
                    #temporay.objects.filter(EmpCode_id=EmpCode,date=current_time.date()).create(login_status="breakin_12-2",Empname=full_name,EmpCode_id=EmpCode,timein_names=EmpCode,timein_timestamps=current_time)
                    messages.success(request, f'TIME IN SUCCESSFULLY!<br> {full_name}', extra_tags='timein_twelve')
                    return HttpResponseRedirect(request.path)

      
            #FOR BREAKOUT
            if "12:00" <= prac_time <= "16:00" and temporay.objects.filter(login_status="breakin_12-2",EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_timein_timestamps >= timedelta(seconds=5):
                    ...
                    messages.success(request, f'BREAK OUT SUCCESSFULLY<br>{full_name}', extra_tags='breakout_twelve')
            ...
            #FOR BREAKIN
            if "12:00" <= prac_time <= "16:00" and temporay.objects.filter(login_status="breakin_12-2",EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakout_timestamps >= timedelta(seconds=8):
                    ...
                    messages.success(request, f'BREAK IN SUCCESSFULLY!<br> {full_name}', extra_tags='breakin_twelve')  
            ...
            if "12:00" <= prac_time <= "15:00" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=8):
                    messages.success(request, f'BREAK IN ALREADY<br>{full_name}', extra_tags='breakin_already_twelve')
                    return HttpResponseRedirect(request.path)


            #FOR TIMEOUT
            if "17:00" <= prac_time <= "23:59" and temporay.objects.filter(login_status="breakin_12-2",EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=5):
                    ...
                    messages.success(request, f'TIME OUT SUCCESSFULLY<br>{full_name}', extra_tags='timeout_twelve')    
            ...
            if "17:00" <= prac_time <= "23:59" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=6):
                    messages.success(request, f'TIMEOUT ALREADY!<br>{full_name}', extra_tags='timeout_already_twelve')
                    return HttpResponseRedirect(request.path)

        
            if "17:01" <= prac_time <= "23:59" and temporay.objects.filter(Q(breakin_names__isnull=True) | Q(breakout_names__isnull=True), EmpCode_id=EmpCode,timein_names__isnull=False, date=current_time.date()).exists():
                nobreak_out_in(EmpCode, current_time)
                messages.success(request, f'NO BREAKIN OR BREAKOUT!<br>{full_name}', extra_tags='no_bibo_utility_twelve')
            ...
            if "17:00" <= prac_time <= "23:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=True, breakin_names__isnull=False, timeout_names__isnull=True, date=current_time.date()).exists():
                existing_entry = temporay.objects.filter(EmpCode_id=EmpCode, date=current_time.date()).first()
...
def breakout(employee_number, current_time):
    ...
    upper_bound_breakout = datetime.combine(current_time.date(), time(12, 00, 0))
...
def timeout(employee_number,current_time):
    ...
    if current_time:
        ...
        upper_bound_timeout = datetime.combine(current_time.date(), time(18, 30, 0))
...
def nobreak_out_in(employee_number, current_time):
    DailyRecord.objects.filter(EmpCode_id=employee_number, date=current_time.date()).update(timeout="00:00:00", absent = "Absent", remarks = "No B-OUT and B-IN" )
...
def utility_mh_render_page(request):
    return render(request, "myapp/utility_mh.html")