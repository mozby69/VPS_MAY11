...
@csrf_exempt
def fetch_messages_serviamus(request):
  
    messages = get_messages(request)
    filtered_messages = [
        {'text': message.message, 'tags': message.tags} for message in messages if 'timein_six' in message.tags
        or 'breakout_six' in message.tags or 'breakin_six' in message.tags or 'timeout_six' in message.tags
        or 'no_bibo_six' in message.tags or 'breakin_already_six' in message.tags or 'timeout_already_six' in message.tags 
        or 'timein_seven' in message.tags or 'breakout_seven' in message.tags or 'breakin_seven' in message.tags
        or 'timeout_seven' in message.tags or 'breakin_already_seven' in message.tags or 'timeout_already_seven' in message.tags
        or 'no_bibo_seven' in message.tags
...
@csrf_exempt
def webcam_qr_code_scanner_serviamus_six(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            # qr_existing = Employee.objects.filter(EmpCode=EmpCode).first()
            # formatted_name = f"{qr_existing.Firstname} {qr_existing.Lastname}" if qr_existing else EmpCode
            ...
            if "04:00" <= prac_time <= "09:59": 
                ...
                if existing_entry is None: 
                    ...
                    #temporay.objects.filter(EmpCode_id=EmpCode,date=current_time.date()).create(login_status="timein 6",Empname=full_name,EmpCode_id=EmpCode,timein_names=EmpCode,timein_timestamps=current_time)
                    messages.success(request, f'TIME IN SUCCESSFULLY!<br> {full_name}', extra_tags='timein_six')
            ...
            #FOR BREAKOUT
            if "11:00" <= prac_time <= "13:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_timein_timestamps >= timedelta(seconds=7):
                    breakout(EmpCode, current_time)  
                    messages.success(request, f'BREAK OUT SUCCESSFULLY<br>{full_name}', extra_tags='breakout_six')
            ...
            #FOR BREAKIN
            if "11:00" <= prac_time <= "13:00" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=True, date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakout_timestamps >= timedelta(seconds=8):
                    ...
                    messages.success(request, f'BREAK IN SUCCESSFULLY!<br> {full_name}', extra_tags='breakin_six')  
            ...
            if "11:00" <= prac_time <= "13:00" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=8):
                    messages.success(request, f'BREAK IN ALREADY<br>{full_name}', extra_tags='breakin_already_six')
                    return HttpResponseRedirect(request.path)
            ...
            if "15:00" <= prac_time <= "23:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, breakout_names__isnull=False,breakin_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=5):
                    ...
                    messages.success(request, f'TIME OUT SUCCESSFULLY<br>{full_name}', extra_tags='timeout_six')    
            ...
            if "15:00" <= prac_time <= "23:59" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=6):
                    messages.success(request, f'TIMEOUT ALREADY!<br>{full_name}', extra_tags='timeout_already_six')
                    return HttpResponseRedirect(request.path)
...
def breakout(employee_number, current_time):
    ...
    upper_bound_breakout = datetime.combine(current_time.date(), time(11, 00, 0))
...
def serviamus_main_page(request):
    return render(request, "myapp/serviamus_six.html")